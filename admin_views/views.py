...
def init_admin_routes():
    ...
    @app.route("/student/delete/<user_id>", methods=["DELETE"])
    @login_required
    ...
    def student_create():
        ...
        groups = Group.query.order_by(Group.number).all()
        return render_template(
            "admin/make_student.html",
            form=form,
            groups=groups,
            redirected_group=redirected_group,
        )
    ...
    def create_auditory():
        ...
        if request.method == "POST" and form.validate():
            auditory = Auditory(
                number=form.number.data, camera_address=form.camera_address.data
            )
    ...
    @app.route("/auditory/delete/<auditory_id>", methods=["DELETE"])
    @login_required
